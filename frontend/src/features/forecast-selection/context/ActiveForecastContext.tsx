...
export function ActiveForecastProvider({ children }: { children: ReactNode }) {
  ...
  const runActiveForecast = useCallback(async () => {
    ...
    try {
      ...
      if (requestId === runIdRef.current) setStatus("ready");
    } catch (e) {
  ...
  const clearActiveForecast = useCallback(() => { setActiveForecastKind("none"); setActiveSessionId(null); setActivePersistedForecastId(null); setActiveForecastBundle(null); setActiveFramesMetadata(null); setSelectedFeature(null); setSelectedFrameIndex(0); setStatus("idle"); setError(null); inFlightRunKeyRef.current = null; }, []);
  const setActiveScenario = useCallback((id: string, label?: string | null) => { setActiveScenarioId(id); setActiveScenarioLabel(label ?? null); setActiveForecastKind("none"); setActiveSessionId(null); setActivePersistedForecastId(null); setActiveForecastBundle(null); setActiveFramesMetadata(null); setSelectedFeature(null); setSelectedFrameIndex(0); setStatus("idle"); setError(null); }, []);
  const setActiveModel = useCallback((modelId: ActiveModelId) => { setActiveModelId(modelId); setActiveForecastKind("none"); setActiveSessionId(null); setActivePersistedForecastId(null); setActiveForecastBundle(null); setActiveFramesMetadata(null); setSelectedFeature(null); setSelectedFrameIndex(0); setStatus("idle"); setError(null); }, []);
