...
const inspectModalSource = source.slice(source.indexOf("{inspectModel ? ("), source.indexOf("function ModelDetailSection"));
...
assert.match(rowMenuSource, /disabled=\{\s*!canDeleteCheckpointFile\(\s*model,\s*activeModelId,\s*\)/s, "row-menu delete is disabled when deletion is unavailable");
assert.match(rowMenuSource, /checkpointDeleteDisabledReason\(\s*model,\s*activeModelId,\s*\)/s, "row-menu delete shows the unavailable reason");
assert.match(rowMenuSource, /onClick=\{\(\) =>\s*void handleDeleteCheckpointFile\(model\)\s*\}/s, "row-menu delete calls the delete handler");
...
assert.match(source, /status === "candidate" \|\|\s*status === "approved" \|\|\s*\(status === "archived" &&\s*ARCHIVED_ACTIVATION_APPROVAL_STATUSES\.has\(approval\)\)/s, "activate gating allows candidate, approved, and previously approved archived models");
...
assert.match(source, /if \(checkpointFileDeletedMetadataPresent\(model\)\)\s*return "Checkpoint file is already deleted\.";/s, "checkpoint_file_deleted true keeps deletion unavailable");
...
assert.match(source, /Checkpoint status/, "Inspect Model keeps checkpoint status in technical details");
